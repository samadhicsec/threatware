#!/usr/bin/env python3
"""
Integration tests for threatware verify CLI action.

These tests verify that the verify action can be invoked without import
...
class TestVerifyAction:
    """Test suite for the verify CLI action."""

    def test_verify_action_available(self, threatware_cli, google_doc_id, template_doc_id, scheme):
        """Test that verify action is available and executes successfully."""
        result = subprocess.run(
            threatware_cli + ["-f", "json", "verify", "-s", scheme, "-d", google_doc_id, "-t", template_doc_id],
            capture_output=True,
            text=True,
            timeout=30
        )
    ...
    def test_verify_help_runs_without_errors(self, threatware_cli):
        """Test that verify --help runs without import errors."""
        result = subprocess.run(
            threatware_cli + ["verify", "--help"],
            capture_output=True,
            text=True,
            timeout=10
        )
    ...
    def test_verify_google_doc_produces_valid_json(self, threatware_cli, google_doc_id, template_doc_id, scheme):
        """Test that verify produces valid JSON output with result field."""
        result = subprocess.run(
            threatware_cli + ["-f", "json", "verify", "-s", scheme, "-d", google_doc_id, "-t", template_doc_id],
            capture_output=True,
            text=True,
        ...
        assert "ModuleNotFoundError" not in result.stderr, \
            f"Module import error in verify: {result.stderr}"
        assert "ImportError" not in result.stderr, \
            f"Import error in verify: {result.stderr}"
    ...
    def test_verify_cli_command_exists(self, threatware_cli):
