...
def verify(verifier_config:dict, model:dict, template_model:dict) -> list:
    ...
    data_tag_models = find.keys_with_tag_matching_regex(model, "^.*-data$")
    ...
    for key_entry, value_entry in find.keys_with_tag_matching_regex(model, "^" + unique_tag_prefix + ".*$"):
        ...
        key_data_tag_name = keymaster.get_data_tag_for_key(key_entry)
        ...
        if not_unique:
            verify_return_list.append(VerifierError(verifier_config, 
                                                    verifier_name, 
                                                    errorTexts["not-unique"].format(key_entry, value_entry, ",".join(section_field_unref_error)), 
                                                    key_entry))
